...
public class User {
	 private String firstname;
	    ...
	    private String encryptPassword() {
	        File file = new File("C:\\Users\\user\\git\\uam\\src\\main\\webapp\\enc1.txt");
	        if (!file.exists()) {
	    ...
	    public String generatefullname(String username) {
	        String fullname="";
	    	try(Connection conn = SampleDb.connect())
	        {
		...
		public void registerUser() {
	        ...
	        try (Connection conn = SampleDb.connect()) {
	            ...
	            try (PreparedStatement checkStmt = conn.prepareStatement(checkQuery);
	                ...
	                if (rs.next() && rs.getInt(1) == 0) {
	                    userType = "admin";
	                    this.firstAdmin=getUsername();
	                }
	            }

	            String query = "INSERT INTO users (firstname, lastname, username, email, password, user_type, Date) VALUES (?, ?, ?, ?, ?, ?, ?)";
	            try (PreparedStatement stmt = conn.prepareStatement(query)) {
	    ...
	    public String login() {
	        try (Connection conn = SampleDb.connect()) {
	            String encPassword = encryptPassword();
	            String query = "SELECT username,user_type FROM users WHERE username = ? AND password = ?";
	            try (PreparedStatement check = conn.prepareStatement(query)) {
	    ...
	    public void managertoadmin(String teamMember) {
	    	
	    	this.newManager=teamMember;
	        try (Connection conn = SampleDb.connect()) {
	            String query = "INSERT INTO requests (username, request_type, request_value, status) VALUES (?, ?, ?, FALSE)";
	            try (PreparedStatement stmt = conn.prepareStatement(query)) {
	                stmt.setString(1, username);
	                stmt.setString(2, "Role Request");
	                stmt.setString(3, "manager-to-admin");
	                stmt.executeUpdate();
	            }
	        } catch (SQLException e) {
	            throw new RuntimeException("Failed to request role: " + e.getMessage(), e);
	    ...
	    public void assignManager(String username) {
	        try (Connection conn = SampleDb.connect()) {
	            String query = "UPDATE users SET user_type = 'manager',managerID=? WHERE username = ?";
	            
	            try (PreparedStatement stmt = conn.prepareStatement(query)) {
	                stmt.setString(1, firstAdmin);
